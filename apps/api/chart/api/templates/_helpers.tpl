...
*/}}
{{- define "node.postgresql.fullname" -}}
...
{{- define "node.secretName" -}}
  ...
  {{- else }}
    {{- printf "%s-%s" .Release.Name "postgresql-binding" | trunc 63 | trimSuffix "-" -}}
  {{- end }}
{{- end -}}
...
{{- if and .Values.postgresql.enabled .Values.externaldb.enabled -}}
node: Database
    You can only use one database.
    Please choose installing a postgresql(R) chart (--set postgresql.enabled=true) or
